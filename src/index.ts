...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Cook County Open Data');
}

const BASE = 'https://datacatalog.cookcountyil.gov';
const UA = 'pipeworx-mcp-data-cookcounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Cook County Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Cook County Open Data dataset by resource_id (e.g. "cjeq-bs86"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "cjeq-bs86" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Cook County Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "cjeq-bs86".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "cjeq-bs86".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"cjeq-bs86"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"cjeq-bs86"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'datacatalog.cookcountyil.gov');
  return res.json();
}
